...
from models.fan import PatchOcclusion
...
def main():
    ...
    if args.resume:
        ...
        if occlusion_model is not None:
            _ = resume_checkpoint(
                occlusion_model, os.path.join(args.output, 'occlusion', 'last.pth.tar'),
    ...
    if args.distributed:
        if has_apex and use_amp != 'native':
            ...
            if occlusion_model is not None:
                occlusion_model = ApexDDP(occlusion_model, delay_allreduce=True)
        else:
            if args.local_rank == 0:
                _logger.info("Using native Torch DistributedDataParallel.")
            model = NativeDDP(model, device_ids=[args.local_rank])  # can use device str in Torch >= 1.1
            if occlusion_model is not None:
                occlusion_model = NativeDDP(occlusion_model, device_ids=[args.local_rank])
    ...
    if args.rank == 0:
        ...
        if occlusion_model is not None:
            save_path_occlusion = os.path.join(output_dir, 'occlusion')
            occlusion_saver = myutils.MyCheckpointSaver(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            if args.use_patch_aug:
                ...
                noise_mask = occlusion_model(input)
                noise = torch.rand(input.shape, dtype=input.dtype, device=input.device)
                noise_sample = input * noise_mask + noise * (1-noise_mask)
                noise_sample = clamp(noise_sample, lower_limit, upper_limit)
                mask_outputs, outputs = model(noise_sample, input)
                loss = loss_fn(outputs, target)
                if args.extra_weight > 0:
                    extra_loss = myutils.FeatureAlignmentLoss(model)
...
def eval_inc(logger, model, device, args=None):
    ...
    result_dict = {}
    ce_alexnet = myutils.get_ce_alexnet()
    ...
    for name, path in myutils.data_loaders_names.items():
    ...
    for name, path in myutils.data_loaders_names.items():
        ...
        acc_top1 /= 5
        CE = myutils.get_mce_from_accuracy(acc_top1, ce_alexnet[name])
        mCE += CE
