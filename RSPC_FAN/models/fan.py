...
# AFAT-FAN-Hybrid Models
@register_model
def afat_fan_tiny_8_p4_hybrid(pretrained=False, **kwargs):
    depth = 8
    sr_ratio = [1] * (depth//2) + [1] * (depth//2 + 1)
    model_args = dict(depths=[3, 3], dims=[128, 256, 512, 1024], use_head=False)
    backbone = _create_hybrid_backbone(pretrained=False, pretrained_strict=False, **model_args)
    model_kwargs = dict(
        patch_size=16, embed_dim=192, depth=depth, num_heads=8, eta=1.0, tokens_norm=True, sharpen_attn=False, **kwargs)
    model = _create_afat('fan_tiny_8_p16_224', pretrained=pretrained, sr_ratio=sr_ratio, backbone=backbone, **model_kwargs)
    return model

@register_model
def afat_fan_small_12_p4_hybrid(pretrained=False, **kwargs):
    depth = 10
    channel_dims = [384] * 10 + [384] * (depth - 10)
    sr_ratio = [1] * (depth//2) + [1] * (depth//2)
    model_args = dict(depths=[3, 3], dims=[128, 256, 512, 1024], use_head=False)
    ...
    model_kwargs = dict(
        patch_size=16, embed_dim=384, depth=depth, num_heads=8, eta=1.0, tokens_norm=True, sharpen_attn=False, **kwargs)
    model = _create_afat('fan_small_12_p16_224', pretrained=pretrained, sr_ratio=sr_ratio, backbone=backbone,
                        channel_dims = channel_dims, **model_kwargs)
    return model

@register_model
def afat_fan_base_16_p4_hybrid(pretrained=False, **kwargs):
    depth = 16
    sr_ratio = [1] * (depth//2) + [1] * (depth//2)
    model_args = dict(depths=[3, 3], dims=[128, 256, 512, 1024], use_head=False)
    backbone = _create_hybrid_backbone(pretrained=False, pretrained_strict=False, **model_args)
    model_kwargs = dict(
        patch_size=16, embed_dim=448, depth=depth, num_heads=8, eta=1.0, tokens_norm=True, sharpen_attn=False, **kwargs)
    model = _create_afat('fan_base_18_p16_224', pretrained=pretrained, sr_ratio=sr_ratio, backbone=backbone, **model_kwargs)
    return model



@register_model
def afat_fan_large_16_p4_hybrid(pretrained=False, **kwargs):
    depth = 22
    sr_ratio = [1] * (depth//2) + [1] * (depth//2)
    model_args = dict(depths=[3, 5], dims=[128, 256, 512, 1024], use_head=False)
    backbone = _create_hybrid_backbone(pretrained=False, pretrained_strict=False, **model_args)
    model_kwargs = dict(
        patch_size=16, embed_dim=480, depth=depth, num_heads=10, eta=1.0, tokens_norm=True, sharpen_attn=False, head_init_scale=0.001, **kwargs)
    model = _create_afat('fan_large_24_p16_224', pretrained=pretrained, sr_ratio=sr_ratio, backbone=backbone, **model_kwargs)
    return model


@register_model
def afat_fan_Xlarge_16_p4_hybrid(pretrained=False, **kwargs):
    """
    For those who have enough GPUs, could try this....
    """
    depth = 23
    ...
    model_kwargs = dict(
        patch_size=16, embed_dim=channel_dims[0], depth=depth, num_heads=num_heads, eta=1.0, tokens_norm=True, sharpen_attn=False, **kwargs)
    model = _create_afat('fan_xlarge_24_p16_224', pretrained=pretrained, sr_ratio=sr_ratio, backbone=backbone,
                        channel_dims = channel_dims, **model_kwargs)
    return model
