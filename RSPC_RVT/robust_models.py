...
class MaskAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., use_mask=False):
        ...
        self.patch_reasoning = nn.Sequential(
            nn.Conv2d(dim, M, K, 1, (K-1)//2, bias=qkv_bias),
            nn.ReLU(inplace=True),
            nn.Conv2d(M, dim*3, 1, bias=qkv_bias)
        )
...
class DecompsableAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., use_mask=False):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        ...
        self.use_mask = use_mask
        if use_mask:
            self.att_mask_1 = nn.Parameter(torch.Tensor(self.num_heads, 14, 14))
            self.att_mask_2 = nn.Parameter(torch.Tensor(self.num_heads, 14, 14))

    def forward(self, x):
        B, N, C = x.shape

        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        qkv = qkv.reshape(3, B, self.num_heads, int(N**0.5), int(N**0.5), C // self.num_heads)
        q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple)

        # first dimension
        dec_q = q.mean(dim=-2)
        dec_k = k.mean(dim=-2)
        attn = (dec_q @ dec_k.transpose(-2, -1)) * self.scale
        if self.use_mask:
            attn = attn * torch.sigmoid(self.att_mask_1).expand(B, -1, -1, -1)
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
        dec_v = v.reshape(B, self.num_heads, int(N**0.5), int(N**0.5) * C // self.num_heads)
        x = attn @ dec_v
        # second dimension
        dec_q = q.mean(dim=-3)
        dec_k = k.mean(dim=-3)
        attn = (dec_q @ dec_k.transpose(-2, -1)) * self.scale
        if self.use_mask:
            attn = attn * torch.sigmoid(self.att_mask_2).expand(B, -1, -1, -1)
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
        dec_v = x.reshape(B, self.num_heads, int(N**0.5), int(N**0.5), C // self.num_heads).transpose(2, 3).reshape(B, self.num_heads, int(N**0.5), int(N**0.5) * C // self.num_heads)
        x = (attn @ dec_v).reshape(B, self.num_heads, int(N**0.5), int(N**0.5), C // self.num_heads).transpose(2, 3)
        x = x.permute(0, 2, 3, 1, 4).reshape(B, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class PoolingTransformer(nn.Module):
    def __init__(self, image_size, patch_size, stride, base_dims, depth, heads,
                 ...
                 attn_drop_rate=.0, drop_rate=.0, drop_path_rate=.0, use_mask=False, masked_block=None, pretrained_cfg=None):
...
class MaskPoolingTransformer(nn.Module):
    def __init__(self, image_size, patch_size, stride, base_dims, depth, heads,
                 mlp_ratio, num_classes=1000, in_chans=3,
                 attn_drop_rate=.0, drop_rate=.0, drop_path_rate=.0, use_mask=False, masked_block=None):
        super(MaskPoolingTransformer, self).__init__()
        ...
        for stage in range(len(depth)):
            ...
            if stage == 0:
                self.transformers.append(
                    MaskTransformer(base_dims[stage], depth[stage], heads[stage],
                                mlp_ratio,
                                drop_rate, attn_drop_rate, drop_path_prob, use_mask=use_mask, masked_block=masked_block)
                )
            else:
...
@register_model
def rvt_tiny_plus_afat(pretrained, **kwargs):
    model = MaskPoolingTransformer(
        image_size=224,
        patch_size=16,
        stride=16,
        base_dims=[32, 32],
...
@register_model
def rvt_small_plus_afat(pretrained, **kwargs):
    model = MaskPoolingTransformer(
        image_size=224,
        patch_size=16,
        stride=16,
...
@register_model
def rvt_base_plus_afat(pretrained, **kwargs):
    model = MaskPoolingTransformer(
        image_size=224,
        patch_size=16,
        stride=16,
